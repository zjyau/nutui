...
function i18n(keysStr, params) {
	...
	let message = { ...(Vue.config.lang === 'zn-CH' ? znCH : {}), ...defaultMessages };
...
function simpleTemplate(templ, conf) {
	...
	if (pars && conf) {
		...
		pars.forEach((c, i) => {
			const reg = new RegExp('{\\s*' + c + '\\s*(?:=\\s*(\\S*?))?\\s*?}', 'g');
			templ = templ.replace(reg, (a, b) => (getArgType(conf[c]) == 'function' ? conf[c]() : conf[c] !== undefined ? conf[c] : b || a));
		});
	}
	return templ;
}
