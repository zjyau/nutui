<template>
	<div class="demo-list">
		<h4>基本用法(选择类)</h4>
		<div>
			<nut-cell :show-icon="true" :is-link="true" @click.native="switchActionSheet('isVisible')">
				<span slot="title"><label>性别</label></span>
				<div slot="desc" class="selected-option">{{ sex }}</div>
			</nut-cell>
			<nut-cell :show-icon="true" :is-link="true" @click.native="switchActionSheet('isVisible1')">
				<span slot="title"><label>年龄</label></span>
				<span slot="sub-title">带取消按钮~~~~</span>
				<div slot="desc" class="selected-option">{{ agespec }}</div>
			</nut-cell>
			<nut-cell :show-icon="true" :is-link="true" @click.native="switchActionSheet('isVisible2')">
				<span slot="title"><label>年龄</label></span>
				<span slot="sub-title">高亮选中项~~~~</span>
				<div slot="desc" class="selected-option">{{ age }}</div>
			</nut-cell>
			<nut-cell :show-icon="true" :is-link="true" @click.native="switchActionSheet('isVisible3')">
		...
		<h4>提示类</h4>
		<div>
			<nut-cell :is-link="true" @click.native="switchActionSheet('isVisible4')">
		...
		<h4>自定义类</h4>
		<div>
			<nut-cell :is-link="true" @click.native="switchActionSheet('isVisible5')">
				<span slot="title"><label>内容自定义</label></span>
				<div slot="desc" class="selected-option">打开</div>
			</nut-cell>
		</div>
		<!-- demo -->
		<nut-actionsheet :is-visible="isVisible" @close="switchActionSheet('isVisible')" :menu-items="menuItems" @choose="chooseItem" />
		<!-- demo(带取消按钮） -->
		<nut-actionsheet
