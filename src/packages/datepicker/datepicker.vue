...
export default {
	...
	methods: {
					...
					this.cacheListData = [
						...[
							...
							this.getChangeMinutes(
								this.defaultValueData[0],
								this.defaultValueData[1],
								this.defaultValueData[2],
								this.defaultValueData[3]
							),
						],
					];
					break;
		...
		resetDefaultValue() {
			...
			const splitArr = cacheDefaultValue.split(' ');
			if (this.type === 'time') {
				const timeArr = splitArr[0].split(':');
		...
		getDays(year, month) {
			...
			const days = Array.from(Array(Utils.getMonthDays(year, month)), (v, k) => {
				if (
		...
		getChangeHours(year, month, day) {
			...
			const hours = Array.from(Array(24).keys()).map((hour) => {
				const startEqualState = year == this.startDateArr[0] && month == this.startDateArr[1] && day == this.startDateArr[2];
				const endEqualState = year == this.endDateArr[0] && month == this.endDateArr[1] && day == this.endDateArr[2];
				const startHour = this.startDateArr[3];
				const endHour = this.endDateArr[3];
		...
		getChangeMinutes(year, month, day, hour) {
			...
			const minutes = Array.from(Array(60).keys()).map((minute) => {
				const startEqualState =
					year == this.startDateArr[0] && month == this.startDateArr[1] && day == this.startDateArr[2] && hour == this.startDateArr[3];
				const endEqualState =
					year == this.endDateArr[0] && month == this.endDateArr[1] && day == this.endDateArr[2] && hour == this.endDateArr[3];
				const startMinute = this.startDateArr[4];
				const endMinute = this.endDateArr[4];
		...
		getHours() {
			...
			const hours = Array.from(Array(parseInt(endHour) + 1), (v, k) => {
		...
		getMinutes() {
			const minutes = Array.from(Array(60), (v, k) => {
				...
				switch (this.type) {
					...
					case 'datetime':
						cacheChooseData.push(
							`${cacheChooseData[0]}-${cacheChooseData[1]}-${cacheChooseData[2]} ${cacheChooseData[3]}:${cacheChooseData[4]}`
						);
						break;
				}
				const week = Utils.getWhatDay(cacheChooseData[0], cacheChooseData[1], cacheChooseData[2]);
		...
		updateLinkage(self, index, value, chooseValue, cacheValueData) {
			...
			switch (index) {
				case 1: // year
					...
					chooseValue = chooseValue || cacheValueData[index];
					const curMonthsData = this.listData[index];
				...
				case 2: // month
					...
					chooseValue = chooseValue || cacheValueData[index];
					const curDaysData = this.listData[index];
					if (curDaysData.indexOf(chooseValue) === -1) {
						if (curDaysData.length < 28) {
							chooseValue = curDaysData[0];
						} else {
							const curChooseDay = parseInt(this.removeChinese(chooseValue));
							const days = curDaysData.length;
				...
				case 3: // day
					...
					chooseValue = chooseValue || cacheValueData[index];
					const curHoursData = this.listData[index];
				...
				case 4: // hour
					...
					chooseValue = chooseValue || cacheValueData[index];
					const curMinuteData = this.listData[index];
