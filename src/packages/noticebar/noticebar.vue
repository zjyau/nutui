<template>
	<div v-show="showNoticeBar" class="nut-noticebar" :class="{ withicon: closeMode, close: closeMode }" :style="barStyle" @click="$emit('click')">
		...
		text: {
			handler() {
				this.$nextTick(() => {
					...
					if (this.scrollable && offsetWidth > wrapWidth && !this.vertical) {
	...
	methods: {
		...
		onAnimationEnd() {
			...
			this.$nextTick(() => {
				...
				this.duration = ((this.offsetWidth + this.wrapWidth) / 375) * this.speed;
