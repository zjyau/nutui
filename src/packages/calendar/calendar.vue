		...
		<div class="nut-calendar" v-show="childIsVisible">
			<div class="nut-calendar-control">
				<span
					...
					>{{ nutTranslate('lang.okBtnTxt') }}</span
				>
							...
							<div class="nut-calendar-month-item">
								<template v-for="(day, i) of month.monthData">
									<div class="nut-calendar-month-day" :class="getClass(day, month)" :key="i" @click="chooseDay(day, month)">
										...
										<span class="nut-calendar-day-tip" v-if="isStartTip(day, month)">{{
											nutTranslate('lang.calendar.start')
										}}</span>
										<span class="nut-calendar-day-tip" v-else-if="isEndTip(day, month)">{{
											nutTranslate('lang.calendar.end')
										}}</span>
									</div>
								</template>
							</div>
...
export default {
	...
	methods: {
		...
		getCurrData(type) {
			const monthData = type == 'prev' ? this.monthsData[0] : this.monthsData[this.monthsData.length - 1];
		...
		getMonth(curData, type) {
			...
			const title = this.nutTranslate('lang.calendar.monthTitle', { year: curData[0], month: curData[1] });
			const monthInfo = {
				curData,
				title,
			...
			if (type == 'next') {
				...
				!this.startData ||
				!Utils.compareDate(`${curData[0]}-${curData[1]}-${curData[2]}`, `${this.startData[0]}-${this.startData[1]}-01`)
			) {
		...
		getClass(day, month, isRange) {
			...
			if (day.type == 'curr') {
				...
				if ((this.startDate && Utils.compareDate(currDate, this.startDate)) || (this.endDate && Utils.compareDate(this.endDate, currDate))) {
			...
			if (this.getClass(day, month, isRange) != `${this.dayPrefix}-disabled`) {
				...
				if (!this.isRange) {
					...
					if (this.currDate.length == 2) {
						this.currDate = [days[3]];
					} else if (Utils.compareDate(this.currDate[0], days[3])) {
					...
					if (this.chooseData.length == 2 || !this.chooseData.length) {
						this.chooseData = [...days];
					} else if (Utils.compareDate(this.chooseData[3], days[3])) {
		...
		touchEnd(event) {
			...
			const updateMove = move + this.transformY;
			const h = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
			const { offsetHeight } = this.$refs.months;
			if (updateMove > 0) {
