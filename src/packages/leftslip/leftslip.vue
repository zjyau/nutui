...
export default {
	...
	mounted() {
		this.$nextTick(() => {
			...
			for (const slot of this.$slots.slipbtns) {
				this.buttonWidth += slot.elm.offsetWidth;
			}
		});
	...
	methods: {
		...
		touchMove(e) {
			...
			const itemWd = this.$refs.slipItem.offsetWidth;
			const wd = this.onlyDelBtn ? 40 : this.buttonWidth;
		...
		restSlide() {
			const listItems = document.querySelectorAll('.nut-leftslip-item');
			...
			if (this.onlyDelBtn) {
				const delBtns = document.querySelectorAll('.delbtn .trans');
