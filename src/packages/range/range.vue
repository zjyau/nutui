	...
	<div class="nut-range">
		...
		<div class="nut-range-box" @click="onClick" :style="{ background: boxColor }">
			...
			<nut-range-bar
				:init-left="initLeft1"
				@getPos="getPosLeft"
				:show-label-always="showLabelAlways"
...
export default {
	...
	methods: {
		updateRangeValues() {
			const rangeValues = this.currentLeft > this.currentRight ? [this.currentRight, this.currentLeft] : [this.currentLeft, this.currentRight];
		...
		setCurrent(posi) {
			...
			const current = (trans / this.cell) * this.cell + this.range[0];
