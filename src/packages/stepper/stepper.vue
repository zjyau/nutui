<template>
	<div :class="{ 'nut-stepper': !simple, 'nut-stepper-simple': simple }">
		<span @click="reduce()" :class="{ 'nut-stepper-grey': isGray }" v-html="require('../../assets/svg/minus.svg')" />
		<input
			type="number"
			:min="minNum"
			:max="max"
		...
		<span @click="add()" :class="{ 'nut-stepper-grey': max && Number(num) > max - step }" v-html="require('../../assets/svg/plus.svg')" />
	</div>
</template>
<script>
export default {
	...
	methods: {
		...
		add() {
			...
			if (this.num <= this.max - this.step && this.max > this.minNum) {
				const [n1, n2] = this.fixedDecimalPlaces(this.num + Number(this.step)).split('.');
				...
				if (this.transition) {
					...
					const f = this.$el.querySelector('.nut-stepper-fake');
