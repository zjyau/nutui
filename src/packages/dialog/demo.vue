<template>
	<div class="demo-list">
		...
		<div>
			...
			<nut-cell :is-link="true" :show-icon="true" @click.native="showDialog2">
				<span slot="title">
					<label>只有标题</label>
				</span>
			</nut-cell>
		...
		<nut-actionsheet :is-visible="isEditor">
			<div slot="custom">
				<nut-cell title="选择货品" :show-icon="true" :is-link="true" @click.native="isItem = true">
					<div slot="desc" class="selected-option">{{ item.name != '' ? item.name : '请选择' }}</div>
				</nut-cell>
				<nut-cell :is-link="true">
					<div slot="title" class="selected-option">
						<nut-textinput v-model="item.num" :has-border="false" placeholder="请输入数量" :clear-btn="true" :disabled="false" />
					</div>
				</nut-cell>
				<nut-cell :is-link="true">
					<div slot="title" class="selected-option">
						<nut-textinput v-model="item.price" :has-border="false" placeholder="请输入价格" :clear-btn="true" :disabled="false" />
					</div>
				</nut-cell>
