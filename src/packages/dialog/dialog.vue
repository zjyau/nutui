<template>
	<div :class="['nut-dialog-wrapper', customClass, { 'nut-dialog-image-wrapper': type === 'image' }]" :id="id">
		<transition :name="animation ? 'nutFade' : ''">
			<div :class="'nut-dialog-mask'" :style="{ background: maskBgStyle }" @click="modalClick" v-show="curVisible" />
		</transition>
		<transition :name="animation ? 'nutEase' : ''">
			<div class="nut-dialog-box" v-show="curVisible" @click="modalClick">
				<div class="nut-dialog" @click.stop>
					<a href="javascript:;" v-if="closeBtn" @click="closeBtnClick" class="nut-dialog-close" />
					...
					<template v-else>
						<div class="nut-dialog-body">
							<span class="nut-dialog-title" v-html="title" v-if="title" />
							<div class="nut-dialog-content" v-if="$slots.default" :style="{ textAlign }">
								<slot />
							</div>
							<div class="nut-dialog-content" v-html="content" v-else-if="content" :style="{ textAlign }" />
						</div>
						<div class="nut-dialog-footer" v-if="!noFooter">
							...
							<button
								...
								@click="okBtnClick"
							>
								{{ okBtnTxt || nutTranslate('lang.okBtnTxt') }}
							</button>
						</div>
					</template>
				</div>
