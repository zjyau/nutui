	...
	<div :class="['nut-searchbar', customClass ? customClass : '']">
		<div class="search-input" :class="[animation ? 'nut-search-ani' : '', inputFocusAnimation ? 'focus' : '']">
			<form action="" id="input-form" @submit="submitFun">
				<nut-icon type="search" v-if="hasIcon" :size="searchIconSize" :color="searchIconColor" />
				<input
					type="text"
					v-model="value"
				...
				<span class="close-icon" :class="hasCloseIcon ? 'show' : ''" @click="clearInput">
					<nut-icon type="circle-cross" :size="clearIconSize" :color="clearIconColor" />
				</span>
			</form>
		</div>
		<a href="javascript:;" class="btn-search" v-if="hasSearchButton" @click="submitFun">
			...
			<nut-icon type="search" v-else :size="searchBtnIconSize" :color="searchBtnIconColor" />
		</a>
	</div>
</template>
<script>
