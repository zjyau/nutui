<template>
	<div class="nut-hor-scroll" ref="wrapper">
		<div class="nut-hor-list" ref="list">
			...
			<slot name="arrow" v-if="$slots.arrow" />
		</div>
	</div>
</template>
<script>
export default {
	...
	methods: {
		isShow() {
			const wrapH = this.$refs.wrapper.clientWidth;
			const listH = this.$refs.list.offsetWidth;
			if (wrapH <= listH) {
		...
		setMove(move, type, time) {
			...
			const w = this.$refs.wrapper.clientWidth;
			const { offsetWidth } = this.$refs.list;
			if (type === 'end') {
				if (updateMove > 0) {
					updateMove = 0;
		...
		touchEvent(changedTouches, callback) {
			...
			const moveY = this.touchParams.lastY - this.touchParams.startY;
			const move = this.touchParams.lastX - this.touchParams.startX;

			if (!(Math.abs(move) > 20 && Math.abs(move) > Math.abs(moveY))) {
				return false;
			}
			const w = this.$refs.wrapper.clientWidth;
			const maxMove = -this.$refs.list.offsetWidth + w;
		...
		touchMove(event) {
			...
			const moveTime = this.touchParams.lastTime - this.touchParams.startTime;
