	...
	methods: {
		...
		shortpwdInput(e) {
			...
			if (this.regExp.test(val)) {
				// 位移假光标
				if (val.length < 6) {
					const num = this.$refs.nutFakeList.offsetLeft + 26;
					const left = val.length * 54 + num;
