<template>
	<div class="nut-tabselect">
		<nut-popup round closeable v-model="isShow" position="bottom" :style="{ height: '457px' }">
			<div class="nut-tabselect-main-title" v-html="mainTitle" />
			<nut-tab @tab-switch="tabSwitchOuter">
				<nut-tab-panel v-for="(value, idx) in tabList" :key="value.tabTitle" :tab-title="value.tabTitle">
					<div class="nut-tabselect-sub-title" v-html="subTitle" />
					<nut-tab @tab-switch="tabSwitchInner" position-nav="left" class="nut-tab-inner">
						<nut-tab-panel v-for="(item, index) in value.children" :key="item.tabTitle" :tab-title="item.tabTitle">
							<ul>
...
export default {
	...
	methods: {
		emit() {
			this.$emit('choose', (this.tabList[this.level0] && this.tabList[this.level0].tabTitle) || '', [...this.allChoose]);
		},
		getText(idx, index, sIndex) {
			const tab = (this.tabList[idx] && this.tabList[idx].children[index]) || {};
			const subTit = tab.tabTitle;
			const content = (tab.content && tab.content[sIndex]) || this.defaultContent[sIndex];
			return subTit + ' ' + content;
		},
		tabSwitchOuter(index, event) {
			this.level0 = index;
