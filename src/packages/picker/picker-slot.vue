<template>
	<div class="nut-picker-list">
		...
		<div class="nut-picker-mask" />
		<div class="nut-picker-indicator" />
	</div>
</template>
<script>
export default {
	...
	methods: {
		...
		setMove(move, type, time) {
			...
			if (type === 'end') {
				...
				const endMove = Math.round(updateMove / this.lineSpacing) * this.lineSpacing;
				const deg = `${(Math.abs(Math.round(endMove / this.lineSpacing)) + 1) * this.rotation}deg`;
		...
		modifyStatus(type, defaultValue) {
			...
			const index = this.listData.indexOf(defaultValue);
			this.currIndex = index === -1 ? 1 : index + 1;
			const move = index === -1 ? 0 : index * this.lineSpacing;
