	...
	<div class="nut-elevator" :style="{ height: wrapHeight + 'px' }">
		<div class="nut-main" :style="{ height: wrapHeight + 'px' }">
			<ul class="nut-elevator-ul" id="nut-elevator-ul">
				<li v-for="item in dataArray" :key="item.title" class="nut-list-title">
...
export default {
	...
	methods: {
		initPage() {
			...
			const initIndex = this.dataArray[this.initIndex].title;
		...
		getFontSize() {
			...
			const bili = this.getStyle(htmlDom, 'fontSize');
		...
		onPointerEnd(e) {
			...
			const navHeight = document.getElementById('nut-elevator-nav').clientHeight;
			const navTop = document.getElementById('nut-elevator-nav').offsetTop;
			const navOffsetTop = navTop - navHeight / 2; // nav距离顶部的距离
			const eTop = e.type.indexOf('touch') !== -1 ? e.changedTouches[0].clientY : e.clientY;
			// let navIndex =parseInt((eTop - navOffsetTop)/this.navHeight/fontSize);
			const navIndex = parseInt((eTop - navOffsetTop) / this.navHeight);
			setTimeout(() => {
				this.currBox = false;
			}, this.hiddenTime);
		...
		onPointerMove(e) {
			...
			const navTop = document.getElementById('nut-elevator-nav').offsetTop;
			const navOffsetTop = navTop - navHeight / 2; // nav距离顶部的距离
			const eTop = e.type.indexOf('touch') !== -1 ? e.touches[0].clientY : e.clientY;
			// let navIndex =parseInt((eTop - navOffsetTop)/this.navHeight/fontSize);
			const navIndex = parseInt((eTop - navOffsetTop) / this.navHeight);
